#############################################
#   LEXER Y PARSER CON ERROR RECOVERY AUTOMÁTICO PLY
#   Versión refactorizada usando mecanismos nativos de PLY
#############################################
...
###############################################################
#    ERROR RECOVERY - VERSIÓN SIMPLIFICADA CON PLY
###############################################################

# Estrategia: Usar el token 'error' en puntos de sincronización clave
# y dejar que PLY maneje automáticamente el descarte de tokens

# 1. Error general en statements - sincroniza en ;
def p_statement_error(p):
    '''statement : error SEMICOL'''
    msg = f"⚠️ RECUPERACIÓN: Error en statement, sincronizando en ';'"
    parser_errors.append(msg)
    print(msg)
    parser.errok()
    p[0] = ('error_stmt',)

# 2. Error en bloque de código - sincroniza en }
def p_body_error(p):
...
# 3. Error en argumentos - sincroniza en )
def p_call_args_error(p):
    '''call_args : error'''
    msg = f"⚠️ RECUPERACIÓN: Error en argumentos"
    parser_errors.append(msg)
    print(msg)
    parser.errok()
    p[0] = []

# 4. Error en parámetros - sincroniza en )
def p_params_error(p):
    '''params : error'''
    msg = f"⚠️ RECUPERACIÓN: Error en parámetros"
    parser_errors.append(msg)
    print(msg)
    parser.errok()
    p[0] = []

# MANEJO GLOBAL DE ERRORES - Aquí está la magia de PLY
def p_error(p):
    global error_count
    ...
    if p:
        ...
        # Buscar el siguiente punto de sincronización (SEMICOL, RBRACE, RPAREN)
        sync_tokens = {'SEMICOL', 'RBRACE', 'RPAREN', 'END'}

        print(f"   🔍 Buscando punto de sincronización...")

        # Descartar tokens hasta encontrar un punto de sincronización
        while True:
            tok = parser.token()
            if not tok:
                print(f"   ⚠️  Llegó al final del archivo")
                break

            if tok.type in sync_tokens:
                print(f"   ✓ Sincronizado en '{tok.type}' (línea {tok.lineno})")
                parser.errok()  # Resetear estado de error
                return tok      # Devolver el token de sincronización
...
print("   📖 Usando recuperación automática de errores de PLY\n")
...
def analizar_archivo(filename):
    ...
    global lexer_errors, parser_errors, error_count
    lexer_errors = []
    parser_errors = []
    error_count = 0
    ...
    print("\n🔍 ANÁLISIS SINTÁCTICO (con recuperación automática PLY):")
    print("-"*70)
...
if __name__ == "__main__":
    print("\n" + "="*70)
    print("COMPILADOR LITTLE DUCK - VERSIÓN CON RECUPERACIÓN AUTOMÁTICA PLY")
    print("="*70 + "\n")
